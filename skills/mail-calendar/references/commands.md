...
```text
node scripts/mailcal.mjs config init --email person@qq.com --mail-provider qq --calendar-provider qq --calendar-user person@qq.com --reuse-mail-secret
```
...
```text
node scripts/mailcal.mjs config show
...
```text
node scripts/mailcal.mjs mail folders
```
...
```text
node scripts/mailcal.mjs mail pending --since 30d --limit 50 --scan-limit 200
```
...
```text
node scripts/mailcal.mjs mail search --since 2026-08-01 --subject 会议 --from example.com --limit 20
node scripts/mailcal.mjs mail search --since 2w --unseen
```
...
node scripts/mailcal.mjs mail get --folder 通知 --uid 123
```
...
node scripts/mailcal.mjs mail ack --uid 125 --uid 126 --outcome processed
```
...
node scripts/mailcal.mjs mail retry --uid 124 --uid 125
...
```text
node scripts/mailcal.mjs calendar create --input event.json
node scripts/mailcal.mjs calendar create --input -
node scripts/mailcal.mjs calendar create --input event.json --calendar-url CALENDAR_COLLECTION_URL
```
...
```text
node scripts/mailcal.mjs calendar delete --uid EVENT_UID
node scripts/mailcal.mjs calendar delete --url EVENT_RESOURCE_URL
```
